...
**Seedance is optional, local fallback by default.** `hasFal()` is true only when `FAL_KEY` is set. Without it `renderStepVideo` returns the `local-storyboard` provider and the canvas plays the same beats, so the studio runs fully local. The fal call is a strict upgrade path, never a hard dependency. Custom guides are parsed from the pasted text and dropped photos (OpenAI when keyed); they no longer fall back to the official LACK sheet.
