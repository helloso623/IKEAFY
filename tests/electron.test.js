...
test("file:// dist talks to the Express API on localhost", () => {
  assert.equal(apiRoot({ protocol: "file:" }), "http://127.0.0.1:8787");
  assert.equal(apiRoot({ protocol: "http:" }), "");
  assert.equal(apiRoot({ protocol: "https:" }), "");
});
